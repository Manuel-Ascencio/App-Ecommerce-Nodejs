...
exports.addProductToCart = catchAsync(async (req, res, next) => {
  ...
  if (quantity > product.quantity) {
    return next(
      new AppError(
        400,
        `There are only ${product.quantity} items of this product`
      )
    );
  }
  ...
  if (!cart) {
    ...
    await ProductInCart.create({ productId, cartId: newCart.id, quantity });
  } else {
    ...
    if (productExists && productExists.status === 'deleted') {
...
exports.updateCartProduct = catchAsync(async (req, res, next) => {
  ...
  if (!productInCart) {
    return next(
      new AppError(404, 'Can not update product, is not in the cart yet')
    );
  }
